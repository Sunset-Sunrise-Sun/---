...
const LAYERS = {
  industry:  { name:'产业空间', color:'#b3452f' },
  life:      { name:'生活服务', color:'#4a7a5c' },
  governance:{ name:'治理节点', color:'#2f6b7d' },
  transit:   { name:'交通连接', color:'#b0863b' },
  outside:   { name:'区域外联', color:'#7a4a6b' }
};

const RELATIONS = [
  { c:'#b3452f', t:'实线 · 布料供应', d:'中大布匹市场 → 村内制衣厂（步行 / 电动车）' },
...
function buildMap(){
  ...
  const C = (l) => (LAYERS[l] ? LAYERS[l].color : '#79705f');
  ...
  s += `<defs>
    <pattern id="hatch" width="8" height="8" patternTransform="rotate(45)" patternUnits="userSpaceOnUse">
      <line x1="0" y1="0" x2="0" y2="8" stroke="rgba(28,26,23,.10)" stroke-width="1"/>
    </pattern>
    <filter id="soft"><feGaussianBlur stdDeviation="6"/></filter>
  </defs>`;

  /* 背景 */
  s += `<rect x="0" y="0" width="1000" height="680" fill="#faf7f0"/>`;

  /* 周边建成区（淡） */
  s += `<rect x="0" y="0" width="1000" height="680" fill="rgba(28,26,23,.018)"/>`;
  s += `<rect x="20" y="20" width="960" height="640" rx="6" fill="none" stroke="rgba(28,26,23,.10)" stroke-dasharray="4 6"/>`;

  /* 中大纺织商圈范围（顶部） */
  s += `<path d="M470 40 L900 40 L900 196 L470 196 Z" fill="rgba(179,69,47,.045)" stroke="rgba(179,69,47,.20)" stroke-dasharray="6 5"/>`;
  s += `<text x="478" y="60" font-size="12" fill="rgba(28,26,23,.45)" font-family="sans-serif">中大纺织商圈 · 59 家专业市场</text>`;
  ...
  s += `<g opacity=".55">` + VILLAGE_LANES.map(d =>
    `<path d="${d}" stroke="rgba(28,26,23,.16)" stroke-width="2.2" fill="none" stroke-linecap="round"/>`).join('') + `</g>`;

  /* 主干道 */
  s += BASE_ROADS.map(r =>
    `<path d="${r.d}" stroke="#fffdf8" stroke-width="${r.w + 5}" fill="none" stroke-linecap="round"/>
     <path d="${r.d}" stroke="rgba(28,26,23,.22)" stroke-width="${r.w}" fill="none" stroke-linecap="round"/>`).join('');
  s += BASE_ROADS.filter(r => r.label).map(r =>
    `<text x="${r.lx}" y="${r.ly}" font-size="11" fill="rgba(28,26,23,.42)" font-family="sans-serif">${r.label}</text>`).join('');

  /* 河涌 */
  s += `<path d="${RIVER}" stroke="rgba(47,107,125,.28)" stroke-width="7" fill="none" stroke-linecap="round"/>`;
  s += `<path d="${RIVER}" stroke="rgba(47,107,125,.55)" stroke-width="1" fill="none" stroke-dasharray="3 4"/>`;
  ...
  s += `<g id="links">` + LINKS.map((l, i) => {
    ...
    return `<path class="mp-link ${l.dashed ? 'dashed' : ''}" data-link="${l.from}-${l.to}"
            ...
            <text x="${mx}" y="${my - 5}" font-size="10" text-anchor="middle"
              fill="rgba(28,26,23,.42)" font-family="sans-serif">${l.label}</text>`;
  }).join('') + `</g>`;
  ...
  s += `<g id="pois">` + POIS.map(p => {
    ...
    g += `<circle class="dot" cx="${p.x}" cy="${p.y}" r="${big ? 8 : 6.5}" fill="${col}" stroke="#faf7f0" stroke-width="2"/>`;
    if (big){
      g += `<text x="${p.x}" y="${p.y + p.r - 12}" text-anchor="middle" font-size="13.5" font-weight="600" fill="#1c1a17">${p.name}</text>`;
  ...
  s += `<g transform="translate(928,600)">
      <circle r="19" fill="rgba(250,247,240,.9)" stroke="rgba(28,26,23,.2)"/>
      <path d="M0 -13 L4.5 4 L0 0.5 L-4.5 4 Z" fill="#b3452f"/>
      <text y="-20" text-anchor="middle" font-size="10" fill="rgba(28,26,23,.5)" font-family="sans-serif">N</text>
    </g>
    <g transform="translate(56,640)">
      <line x1="0" y1="0" x2="120" y2="0" stroke="rgba(28,26,23,.45)" stroke-width="1.5"/>
      <line x1="0" y1="-5" x2="0" y2="5" stroke="rgba(28,26,23,.45)" stroke-width="1.5"/>
      <line x1="120" y1="-5" x2="120" y2="5" stroke="rgba(28,26,23,.45)" stroke-width="1.5"/>
      <text x="60" y="-8" text-anchor="middle" font-size="10.5" fill="rgba(28,26,23,.5)" font-family="sans-serif">示意距离 · 非等比例</text>
    </g>`;
...
function resetPanel(){
  ...
  box.innerHTML = `
    ...
    <h3>点击任一节点</h3>
    <p class="psum">地图上共标注 15 处空间节点，覆盖产业、生活、治理、交通与区域外联五个图层。</p>
    <div class="pdetail">
      这张图不追求测绘精度，而是一张<b>关系地图</b>：它想表达的是供应、用工、出货、治理这四张网络，
      如何被压缩在同一平方公里之内。也正因如此，当产业需要外迁时，被拆散的是一整套关系，而不只是几栋房子。
    </div>
    <ul class="fact-list">
      <li><span class="k">标注节点</span><span class="v">15 处</span></li>
      <li><span class="k">图层</span><span class="v">5 类</span></li>
